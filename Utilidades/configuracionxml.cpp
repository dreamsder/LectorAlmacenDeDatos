...
QString ConfiguracionXml::m_perfil;
...
bool ConfiguracionXml::leerConfiguracionXml(){
    ...
    if(QDir::rootPath()=="/"){
        filename = "/opt/LectorAlmacen/conf.xml";
    }else{
    ...
    if(QFile::exists(filename)){
        ...
        while(!read.atEnd()){
            ...
            if(read.isStartElement())
            {
                ...
                }else if(read.name() == "ACCESO_PERFIL"){

                    read.readNext();

                }else if(read.name() == "tipoperfil"){
                    //Los perfiles disponibles son:
                    //toor: acceso completo
                    //ocincet: acceso para tecnicos
                    ConfiguracionXml::setPerfil(read.readElementText());
                }
                else{
                    read.readNext();
                }
